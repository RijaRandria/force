...
class MonController extends Controller
{
  ...
  public function Renault(RequestInterface $request, ResponseInterface $response)
  {
    ...
    $this->render($response, 'nfa017/vehicules.twig', ['titre'=>'Véhicule de marque Renault', 'lignes'=> $lignes]);
  }
  ...
  public function Citroen(RequestInterface $request, ResponseInterface $response)
  {
    ...
    $lignes = $requete->fetchAll();
    $this->render($response, 'nfa017/vehicules.twig', ['titre'=>'Véhicule de marque Citroen', 'lignes'=> $lignes]);
  }
  ...
  public function  Audi(RequestInterface $request, ResponseInterface $response)
  {
    ...
    $lignes = $requete->fetchAll();
    $this->render($response, 'nfa017/vehicules.twig', ['titre'=>'Véhicule de marque Audi', 'lignes'=> $lignes]);
  }
  ...
  public function  Skoda(RequestInterface $request, ResponseInterface $response)
  {
    ...
    $lignes = $requete->fetchAll();
    $this->render($response, 'nfa017/vehicules.twig', ['titre'=>'Véhicule de marque Skoda', 'lignes'=> $lignes]);
  }

  // Méthode de test TWIG
  public function Vide(RequestInterface $request, ResponseInterface $response)
  {
    $this->render($response, 'nfa017/vide.twig', ['titre'=>'Cette page est vide']);
  }

  // Méthode de Recherche
  public function Search(RequestInterface $request, ResponseInterface $response)
  {
    $this->render($response, 'nfa017/recherche.twig', ['titre'=>'Recherche par marque, véhicule ou moteur']);
  }
  ...
  public function Requete(RequestInterface $request, ResponseInterface $response)
  {
    ...
    $requete = $this->pdo->prepare("SELECT marque,vehicule,moteur FROM vehicules WHERE marque = ? OR vehicule = ? OR moteur = ?");
    $requete->execute(array($recherche, $recherche, $recherche));
    $lignes = $requete->fetchAll();
    $this->render($response, 'nfa017/vehicules.twig', ['titre'=>'Résultats par marque, véhicule ou moteur', 'lignes'=> $lignes]);
  }
