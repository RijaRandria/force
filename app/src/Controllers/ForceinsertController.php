...
class ForceinsertController extends Controller
{
 ...
 // Méthode inmodifsalform
  // permet de mettre à jour la formation salarié
  public function inmodifsalform(RequestInterface $request, ResponseInterface $response,$args)
  {
    $idcqp=$args['idcqp'];
   
     $requete = $this->pdo->prepare(" UPDATE cqp 
                                      SET inscrit = ?,
                                          motifnon = ?,
                                          nbheures = ?,
                                          valid = ?,
                                      ...
                                      WHERE idcqp = $idcqp");
 $requete->execute([
  $request->getParsedBody()['inscrit'],
  $request->getParsedBody()['motifnon'],
 ...
 $this->render($response, 'force/salarieformation.twig', ['titre'=>'Résultats' , 'lignes'=> $lignes]);
}
